            ...
            for row in reader:
                try:
                    ...
                    Reserva.objects.create(
                        ...
                        precio_por_noche=(
                            parse_float(row["Precio por noche"])
                            if row["Precio por noche"]
                            else 0.0
                        ),
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"Reserva de {row['Nombre']} {row['Apellido']} importada correctamente."
                        )
                    )
                except Exception as e:
                    self.stderr.write(
                        self.style.ERROR(
                            f"Error al importar la reserva de {row['Nombre']} {row['Apellido']}: {e}"
                        )
                    )
